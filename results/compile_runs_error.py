...
from plotters.plot_utils import compute_total_RMSE, gen_all_data_and_plot, compute_day_RMSE
    ...
    for i, config in enumerate(config_dirs):
        ...
        for j in range(num_days):
            ...
            """if "grape_phenology" in config.DataConfig.dtype:
                ...
                all_avg_pheno[:, j, i] = np.concatenate(
                    (
                        train_avg[1:-1],
            ...
            if "grape_phenology" in config.DataConfig.dtype:
                ...
                val_day_rmse = np.array(
                    [
                        compute_day_RMSE(true_data[1], output_data[1], all_inds[1], days=k * days_step)
                        for k in range(num_days)
                    ]
                )
                test_day_rmse = np.array(
                    [
                        compute_day_RMSE(true_data[2], output_data[2], all_inds[2], days=k * days_step)
                        for k in range(num_days)
                    ]
                )
                    ...
                    for si in range(calibrator.num_sites):
                        for k in range(calibrator.num_cultivars):
                            ...
                            """if "grape_phenology" in config.DataConfig.dtype:
                                ...
                                all_cultivar_avg_pheno[r, s, si, k, :, j, i] = np.concatenate(
                                    (
                            ...
                            if "grape_phenology" in config.DataConfig.dtype:
                                ...
                                all_cultivar_avg_pheno[r, s, si, k, :, j, i] = np.array(
                                    np.concatenate(
                                        [
                                            cult_day_rmse,
                                            [cultivar_train_rmse],
