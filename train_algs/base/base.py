...
import numpy as np
import random
from train_algs.base.process_data import process_data_valset, process_data_novalset
from train_algs.base.util import StageMSELoss, StageCrossEntropyLoss, ScaledMSELoss
...
class BaseModel(nn.Module):
    ...
    def make_optimizer(self, model: nn.Module) -> None:
        ...
        self.p = self.config.DConfig.pinn_p

        if self.config.DConfig.loss_func == "MSELoss":
            self.loss_func = nn.MSELoss(reduction="none")
        elif self.config.DConfig.loss_func == "CrossEntropyLoss":
            self.loss_func = nn.CrossEntropyLoss(reduction="none")
        elif self.config.DConfig.loss_func == "ScaledMSELoss":
            self.loss_func = ScaledMSELoss(weight=self.config.DConfig.scale)
        elif self.config.DConfig.loss_func == "BudbreakCrossEntropyLoss":
            self.loss_func = StageCrossEntropyLoss("Budbreak", reduction="none")
            self.loss_func_mse = StageMSELoss("Budbreak", reduction="none")
        elif self.config.DConfig.loss_func == "BloomCrossEntropyLoss":
            self.loss_func = StageCrossEntropyLoss("Bloom", reduction="none")
            self.loss_func_mse = StageMSELoss("Bloom", reduction="none")
        elif self.config.DConfig.loss_func == "VeraisonCrossEntropyLoss":
            self.loss_func = StageCrossEntropyLoss("Veraison", reduction="none")
            self.loss_func_mse = StageMSELoss("Veraison", reduction="none")
        elif self.config.DConfig.loss_func == "BudbreakMSELoss":
            self.loss_func = StageMSELoss("Budbreak", reduction="none")
        elif self.config.DConfig.loss_func == "BloomMSELoss":
            self.loss_func = StageMSELoss("Bloom", reduction="none")
        elif self.config.DConfig.loss_func == "VeraisonMSELoss":
            self.loss_func = StageMSELoss("Veraison", reduction="none")
        else:
            msg = f"Unimplemented DConfig Loss Function {self.config.DConfig.loss_func}"
