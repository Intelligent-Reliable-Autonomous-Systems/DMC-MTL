...
from plotters.plot_utils import C_AGG, C_PER, C_SHAPES
...
def main():
    ...
    parser.add_argument(
        "--wheat_cultivars",
        default=["Winter_Wheat_101", "Winter_Wheat_105", "Winter_Wheat_107", "Apache", "Bermude"],
        type=list,
        help="Path to Config",
    )
    ...
    for c in args.pheno_cultivars:
        with open(f"_data/processed_data/grape_phenology_{c}.pkl", "rb") as f:
            data = pkl.load(f)[-args.yr]
            pheno_yr.append(data)
        f.close()
    for c in args.ch_cultivars:
        with open(f"_data/processed_data/grape_coldhardiness_{c}.pkl", "rb") as f:
            data = pkl.load(f)[-args.yr]
            ch_yr.append(data)
        f.close()
    for c in args.wheat_cultivars:
        with open(f"_data/processed_data/wofost_{c}.pkl", "rb") as f:
            data = pkl.load(f)[-args.yr]
            wf_yr.append(data)
        f.close()

    fig, ax = plt.subplots(nrows=3, figsize=(6, 5.25))
    ...
    for i in range(len(args.wheat_cultivars)):
        ax[2].plot(
            np.arange(len(wf_yr[i])),
            wf_yr[i].loc[:, "WSO"],
            label=args.wheat_cultivars[i].replace("_", " ").replace("Winter ", ""),
            c=C_PER[i],
            alpha=0.9,
        )
    ax[2].set_ylim(ymin=-5)
    ...
    ax[2].text(0.99, 0.91, f"(c)", ha="right", va="top", transform=ax[2].transAxes, fontsize=fontsize + 1)
    ...
    plt.savefig("_data/figs/visualization_of_data.pdf", bbox_inches="tight")
    plt.close()
