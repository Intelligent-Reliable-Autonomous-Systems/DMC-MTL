...
def SWEAF(ET0: torch.Tensor, DEPNR: torch.Tensor) -> torch.Tensor:
    ...
    sweaf = 1.0 / (A + B * ET0).clamp(min=EPS) - (5.0 - DEPNR) * 0.10

    sweaf = torch.where(DEPNR < 3.0, sweaf + (ET0 - 0.6) / (DEPNR * (DEPNR + 3.0)), sweaf)
...
class EvapotranspirationCO2_TensorBatch(BatchTensorModel):
    ...
    def __call__(
        ...
        r.RFWS = torch.clamp((SM - p.SMW) / (SMCR - p.SMW).clamp(min=EPS), self.zero_tensor, self.one_tensor)

        RFOSMX = torch.clamp((p.SM0 - SM) / p.CRAIRC.clamp(min=EPS), self.zero_tensor, self.one_tensor)
        r.RFOS = torch.where(
            (p.IAIRDU == 0) & (p.IOX == 1),
            RFOSMX + (1.0 - torch.min(DSOS, torch.tensor([4]).to(self.device)) / 4.0) * (1.0 - RFOSMX),
        ...
        r.IDWS = torch.where(r.RFWS < 1.0, 1.0, r.IDWS)
        self._IDWST = torch.where(r.RFWS < 1.0, self._IDWST + 1, self._IDWST)

        r.IDOS = torch.where(r.RFOS < 1.0, 1.0, r.IDOS)
        self._IDOST = torch.where(r.RFOS < 1.0, self._IDOST + 1, self._IDOST)
    ...
    def reset(self, day: date, inds: torch.Tensor = None) -> None:
        ...
        r.EVWMX = torch.where(inds, 0.0, r.EVWMX).detach()
        r.EVSMX = torch.where(inds, 0.0, r.EVSMX).detach()
        r.TRAMX = torch.where(inds, 0.0, r.TRAMX).detach()
        r.TRA = torch.where(inds, 0.0, r.TRA).detach()
        r.IDOS = torch.where(inds, 0.0, r.IDOS).detach()
        r.IDWS = torch.where(inds, 0.0, r.IDWS).detach()
        r.RFWS = torch.where(inds, 0.0, r.RFWS).detach()
        r.RFOS = torch.where(inds, 0.0, r.RFOS).detach()
        r.RFTRA = torch.where(inds, 0.0, r.RFTRA).detach()

        self._IDOST = torch.where(inds, 0.0, self._IDOST).detach()
        self._IDWST = torch.where(inds, 0.0, self._IDWST).detach()
    ...
    def get_extra_states(self) -> dict[str, torch.Tensor]:
        ...
        Get extra states
        """
        return {"_IDWST": self._IDWST, "_IDOST": self._IDOST}
