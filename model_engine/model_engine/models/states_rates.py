...
class TensorAfgen(object):
    ...
    def _check_x_ascending(self, tbl_xy: list) -> tuple[list, list]:
        ...
        if sum_break == 0:
            ...
            msg = "X values for AFGEN input list not strictly ascending: %s" % x_list
...
class TensorBatchAfgen(object):
    ...
    def _check_x_ascending(self, batch_tbl_xy: list) -> tuple[np.ndarray, np.ndarray]:
        """Checks that the x values are strictly ascending."""
        x = []
        y = []
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        v = torch.where(
            ...
            torch.where(
                x >= self.x_list[:, -1],
                self.y_list[:, -1],
                self.y_list[i, j] + self.slopes[i, j] * (x.squeeze() - self.x_list[i, j]),
            ),
        )  # Slopes
        return v
...
class TensorAfgenTrait(TraitType):
    ...
    def validate(self, obj: object, value: Iterable | TensorAfgen) -> TensorAfgen:
...
class TensorBatchAfgenTrait(TraitType):
    ...
    def validate(self, obj: object, value: Iterable | TensorBatchAfgen) -> TensorBatchAfgen:
